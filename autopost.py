...
class AutoContentSEO:
        ...
        if serp_key:
            try:
                ...
                for t_url in target_urls:
                    ...
                    if content: 
                        self.add_log(log_placeholder, f"Đã học hỏi xương sống từ đối thủ thành công.", "success")
                        return content
            except: pass
        return None
    ...
    def step1_kiem_tra_he_thong(self, log_placeholder) -> bool:
        ...
        self.add_log(log_placeholder, f"Đã chốt Website: {self.target_web.get('WS_NAME')}", "success")
        return True

    def run_ai_content_pipeline(self, log_placeholder):
        self.add_log(log_placeholder, "Bắt đầu nhịp nặn chữ...", "info")
        ...
        ref_content = self.fetch_reference_content(log_placeholder) or "Tự do sáng tạo chuyên sâu."
        
        prompt = f"Viết bài SEO HTML về {self.main_kw_text}, dài {self.final_word_count} chữ. Keywords: {', '.join(self.content_kws)}. Trả về HTML (h1, h2, h3, p). Không in đậm từ khoá."
        ...
        return {
            ...
            'REP_KW_2': self.all_used_kws[1],
            'REP_KW_3': self.all_used_kws[2],
...
with tab2:
    ...
    with col_l:
        ...
        force_btn = st.button("✈️ ÉP ĐĂNG BÀI PENDING")
