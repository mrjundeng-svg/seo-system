...
class AutoSEOPipeline:
    ...
    def step4_llm_generation(self, ui_log) -> bool:
        ...
        1. CẤM TUYỆT ĐỐI các từ ngữ chào hỏi/dẫn dắt ở đầu bài: "Kính thưa các Sếp", "Chào quý vị", "Thân gửi", "Tuyệt vời". VÀO THẲNG VẤN ĐỀ BẰNG ĐOẠN SAPO.
        2. TIÊU ĐỀ (THẺ H1): Bắt buộc chứa cụm từ "{main_kw}". Nằm NGẪU NHIÊN ở GIỮA hoặc CUỐI tiêu đề (TUYỆT ĐỐI KHÔNG ĐẶT Ở ĐẦU CÂU).
        3. TỪ KHÓA TRONG BÀI:
        ...
        4. TỔNG SỐ CHỮ: Chính xác khoảng {self.target_length} chữ. KHÔNG LẶP CẤU TRÚC BÀI CŨ.
        ...
        # ĐÃ FIX: Lột sạch dấu in đậm Markdown do AI sinh ra để không cản trở việc gắn Link
    ...
    def step5_6_spin_and_dom(self, ui_log):
        ...
        # ĐÃ FIX SPAM LINK ĐÁY BÀI: Rải tự nhiên vào các đoạn văn khác nhau
        if missed_kws:
            ...
            for k, u, is_e in missed_kws:
                if avail_p:
                    ...
                    avail_p.remove(target_p) # Rút ra khỏi mảng để không chèn 2 link vào 1 đoạn
...
with tab1:
    ...
    # ĐÃ SỬA: CÁC NÚT BẤM LUÔN HIỂN THỊ, KHÔNG BỊ BIẾN MẤT
