...
def post_to_cms(website_row, title, html_content, dash_config):
    ...
    if "@blogger.com" in blog_receiver_email.lower():
        ...
        try:
            ...
            server.quit()
            return True, f"Bắn Blogspot thành công -> Tới: {blog_receiver_email}"
        except Exception as e:
...
class AutoSEOPipeline:
    ...
    def step1_allocate_slot(self, ui_log) -> bool:
        ...
        for d_off in range(max_days + 1):
            ...
            for _, web in avail_webs.iterrows():
                ...
                self.add_log(ui_log, f"🔍 [CHECK QUOTA] Local '{ws_name}' ({day_x_str}): {len(posts_day_x)}/{ws_limit}", "quota")
    ...
    def step4_llm_generation(self, ui_log) -> bool:
        ...
        for k in req_keys: 
            snippet = (prompts[k][:150] + '...') if len(prompts[k]) > 150 else prompts[k]
        ...
        6. GÓC NHÌN ĐỘC NHẤT: Bắt buộc triển khai bài viết theo một góc nhìn hoặc cách dẫn dắt hoàn toàn khác biệt so với thông thường. (Mã Seed Sáng Tạo: {seed_sang_tao}).
        ...
        for gk in gem_keys:
            ...
            for gm in gem_models:
                ...
                except Exception as e: self.add_log(ui_log, f"⚠️ Gemini sập (Lỗi 429/503). Đang chuyển dự phòng...", "warn")
