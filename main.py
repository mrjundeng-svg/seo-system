...
class AutoSEOPipeline:
        ...
        for d_off in range(max_days + 1):
            ...
            for _, web in avail_webs.iterrows():
                ...
                if len(day_posts) < ws_limit:
                    ...
                    self.add_log(ui_log, f"✅ [CHỐT SLOT] {ws_name} | {pub_t.strftime('%H:%M %d/%m/%Y')}", "success")
                    return True
        return False
    ...
    def step2_3_keyword_and_serp(self, ui_log) -> bool:
        ...
        mn, mx = self.parse_rng(self.dashboard.get('WORD_COUNT_RANGE', '900-1200').split('-')[0], 900), self.parse_rng(self.dashboard.get('WORD_COUNT_RANGE', '900-1200').split('-')[-1], 1200)
        ...
        if s_key:
            try:
                ...
                if res:
                    rh = requests.get(res[0]["link"], headers={'User-Agent': 'Mozilla/5.0'}, timeout=15)
                    if rh.status_code == 200:
                        self.serp_style = "\n".join([t.get_text(strip=True) for t in BeautifulSoup(rh.text, 'html.parser').find_all(['h2', 'h3', 'p'])])[:3000]
                        self.add_log(ui_log, f"✅ [SERP] Cào văn phong thành công.")
            except: pass
        return True
    ...
    def step4_llm_generation(self, ui_log) -> bool:
        ...
        for i, k in enumerate(self.all_kws): self.raw_html = re.sub(rf'\[?REP_KW_{i+1}\]?', k, self.raw_html, flags=re.IGNORECASE)
        self.raw_html = re.sub(r'\{\{keyword\}\}', self.all_kws[0], self.raw_html, flags=re.IGNORECASE)
    ...
    def step5_6_spin_and_dom(self, ui_log):
        ...
        if missed:
            ...
            for k, u in missed:
                pfx = random.choice(["Hơn nữa, Sếp có thể xem thêm về", "Một lựa chọn tốt là", "Thông tin hữu ích về"])
                if avail_p:
                    target = random.choice(avail_p); avail_p.remove(target)
                    target.append(BeautifulSoup(f" {pfx} <a href='{u}'>{k}</a>.", 'html.parser'))
        ...
        self.add_log(ui_log, f"🖼️ [QUOTA ẢNH] Web giới hạn tối đa {mx_img} ảnh.", "detail")
        ...
        self.add_log(ui_log, f"🖼️ [GẮN ẢNH] Thành công {len(self.used_imgs)} ảnh."); self.raw_html = str(soup); return True
    ...
    def step7_qa_validation(self, ui_log) -> str:
        ...
        return "PENDING" if seo >= req and ai <= 20 and rd >= 60 else "FAIL"
