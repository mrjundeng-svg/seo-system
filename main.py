...
for i, (name, cols) in enumerate(TABS_CONFIG.items()):
    with tabs[i]:
        with st.expander(f"📥 NHẬP LIỆU NHANH {name} (Dán từ Excel vào đây)"):
            raw_data = st.text_area("Dán nội dung (Tab-separated):", key=f"txt_{name}", height=150)
            if st.button("🔥 ĐỔ VÀO BẢNG", key=f"load_{name}") and raw_data:
                try:
                    st.session_state['db'][name] = pd.read_csv(io.StringIO(raw_data), sep='\t', names=cols)
                    st.rerun()
                except:
                    st.error("Lỗi định dạng! Vui lòng copy đúng số cột.")
