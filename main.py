...
# --- 1. SETUP & MẮT THẦN DỮ LIỆU ---
st.set_page_config(page_title="LAIHO.VN - BATCH SEO MASTER", layout="wide")
...
# --- 2. HÀM GỌI AI (VỚI CƠ CHẾ CHỐNG NGHẼN) ---
...
# --- 3. TIẾN TRÌNH VIẾT BÀI HÀNG LOẠT ---
...
def run_batch_popup(all_data, sh, num_posts):
    ...
    # Lọc danh sách từ khóa chưa làm (Status = 0 hoặc trống)
    ...
    for i, (idx, row) in enumerate(todo_list.iterrows()):
        ...
        status_text.write(f"正在 viết bài {i+1}/{len(todo_list)}: **{kw_main}**")
        ...
        if "❌" not in content:
            ...
            ws_kw.update_cell(cell.row, cell.col + 2, "SUCCESS") # Giả định cột C là Status
            
            st.success(f"✅ Đã xong: {kw_main}")
            # Chờ 3-5s để tránh bị Groq chặn vì gọi quá nhanh (Rate limit)
            time.sleep(3)
...
if data:
    ...
    num_to_write = st.sidebar.number_input("Số lượng bài cần viết", min_value=1, max_value=20, value=5)
    
    c1, c2, _ = st.columns([1.5, 1, 4])
    with c1:
        if st.button(f"Viết {num_to_write} bài AI", type="primary", use_container_width=True):
            run_batch_popup(data, sh, num_to_write)
    with c2:
        if st.button("🔄 Làm mới dữ liệu", use_container_width=True):
            st.cache_data.clear(); st.rerun()

    st.divider()
    st.subheader("📂 Danh sách từ khóa chờ xử lý")
